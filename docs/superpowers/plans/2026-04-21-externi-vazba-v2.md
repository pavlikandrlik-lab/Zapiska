...
> **AKTUALIZACE 2026-04-22:** Task 10 (`IHarvestScheduler` stub) zůstává validní — rozhraní, call-site v `RecordService.SaveRecord` + `NoOpHarvestScheduler` implementace. **Implementace** se však přesune: NoOp zůstane dokud nebude hotový plán `2026-04-22-sync-infra-and-ad.md` + navazující revize [`2026-04-22-sd-sync-revise.md`](2026-04-22-sd-sync-revise.md). Pak se NoOp nahradí **`ReactiveHarvestSchedulerAdapter`** (viz sd-sync-revise Task 4), který jen zapíše do `IReactiveSyncQueue<SdReactiveHarvestRequest>`. Signatura `IHarvestScheduler` zůstává, mění se jen jeho implementace.
>
