...
/// </summary>
public sealed class SdArchiveSyncJobAdminHandler : ISyncJobAdminHandler
{
    private static readonly TimeSpan ManualFloor = TimeSpan.FromMinutes(1);

    private readonly PmTrackerDbContext _db;
    private readonly ISyncJobRunLock<SdArchiveSyncSettingsEntity> _runLock;
    private readonly ManualTriggerSignal<SdArchiveSyncSettingsEntity> _manualSignal;
    ...
    public string JobKey => "sd.archive";

    public async Task<SyncJobSettingsCardViewModel> LoadCardAsync(bool canManage, CancellationToken ct)
    {
        var s = await _db.SdArchiveSyncSettings.AsNoTracking().FirstAsync(x => x.Id == 1, ct);
        ...
        return new SyncJobSettingsCardViewModel
        {
            ...
            Description = "Pravidelné načítání vyjádření z HOT_VYJADRENI pro archivované tickety (stav = 'archiv'). Běží méně často.",
    ...
    public async Task<bool> SaveAsync(SyncJobSettingsInputModel input, int? editorOsobaId, CancellationToken ct)
    {
        ...
        var s = await _db.SdArchiveSyncSettings.FirstAsync(x => x.Id == 1, ct);
    ...
    public async Task<ManualRunOutcome> TriggerManualRunAsync(int? editorOsobaId, CancellationToken ct)
    {
        ...
        var settings = await _db.SdArchiveSyncSettings.AsNoTracking().FirstAsync(x => x.Id == 1, ct);
